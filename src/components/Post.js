...
const Post = ({
  ...
  useEffect(() => {
    if (list[id] === undefined) {
      dispatch(likeActions.setLike(id));
    }
  }, []);
  ...
  const handleClick = (id) => {
    ...
    if (toggleLike !== true) {
      dispatch(likeActions.addLiketFB(id, list[id]));
    } else {
      dispatch(likeActions.deleteLiketFB(id, list[id]));
    }
  };
  return (
    <React.Fragment>
      <Grid margin='20px 0' bg='white' height='100%'>
        <Grid isFlex>
          <Grid padding='8px' isFlex width='none'>
            <Image shape='circle' src={src} _onClick={_onClick} />
            <Text bold>{userInfo.userName}</Text>
          </Grid>
          <Grid margin='0 0 0 120px' width='none'>
            {isMe && isEdit && (
              <Button text='수정' padding='4px' _onClick={_onClick}></Button>
            )}
            <Grid margin='0 0 0 120px' width='none'>
              <Text>{insertDt}</Text>
            </Grid>
        ...
        {Layout1 && (
          <React.Fragment>
            ...
            <Grid>
              <Image shape='rectangle' src={imageUrl} _onClick={_onClick} />
            </Grid>
            <Grid isFlex padding='8px'>
              <Text bold>댓글 {commentCnt}개</Text>
              <div style={{ display: 'flex' }}>
                <Text bold>좋아요 {list[id]}개</Text>
                <Image
                  shape='none'
                  src={
                    toggleLike
